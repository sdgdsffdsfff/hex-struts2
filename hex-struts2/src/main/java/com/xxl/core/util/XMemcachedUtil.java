	...
	private static MemcachedClient getInstance() {
		if (memcachedClient == null) {
			synchronized (MemcachedClient.class) {
				// 构建client分布式权重参数
				//String serverAddress = "localhost:11211 localhost:11211";
				//int[] weights = { 1, 2};
				
				PropertiesUtil.init("memcached.properties");
				String serverAddress = PropertiesUtil.getProperty("server.address", "").replace(",", " ");
				
				String[] weightsArr = PropertiesUtil.getProperty("server.weights", "").split(",");
